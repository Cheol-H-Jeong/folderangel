...
한 사업에서 발생한 파일이라도 다음 용도가 섞이면 별도 폴더로 분리한다:
  · *산출물* (제안서·보고서·발표·결과물 등 사업 주된 작업물)
  · *구매·계약* (발주서, 계약서, 견적서, 거래명세서, GPU/장비 구매 등)
  · *회계·세금* (세금계산서, NTS_eTaxInvoice, 영수증, 정산서, 비용 처리)
  · *회의·일정* (회의록, 일정 안내, 공지사항)
  · *미디어·이미지·영상* (.mp4 / .mov / .jpg / .jpeg / .png / IMG_*, 카메라
    자동 명명 사진, 강의·발표 녹화 등 — 본문 파싱이 사실상 불가능한 자료)
  · *참고 자료* (외부 보고서, 표준안, 논문 PDF 등)
한 사업 안에 여러 용도의 파일이 있으면 사업 폴더와 용도 폴더가 모두 만들어져도 좋다.
세금계산서와 GPU 구매계약을 사업 산출물 폴더에 절대 넣지 마라 — 별도 회계/계약 폴더로.
...
너는 두 가지 일을 동시에 한다:
1) 파일명만 보고 (a) 사업/과제/프로젝트/기관 축 + (b) 목적·용도 축
   (산출물 / 구매·계약 / 회계·세금 / 회의·일정 / 미디어·이미지·영상 /
   참고 자료) 양쪽을 함께 고려해 카테고리를 설계한다.
2) 각 파일에 대해 다음 중 하나를 선택한다:
   (a) **파일명만으로 어느 카테고리인지 확신**할 수 있다 → assignments 에 넣는다.
       파일명에 사업명/고객사/시스템/제품/약어 등 식별자가 분명히 보이는 경우.
   (b) **파일명이 너무 일반적**이다 (예: "최종.pptx", "보고서.docx",
       "재무제표.xlsx", "약관.pdf", "공지사항.docx") → assignments 에 넣지
       말고 **deferred** 배열에 그 파일의 path 그대로 넣는다.

**즉시 deferred 로 보내야 할 파일명 패턴 (분류 시도 자체 금지):**
  · 순수 숫자: 예) `1152.PDF`, `1767000341906.pdf`
  · 랜덤 해시 / 인코딩 문자열: 예) `4aL6Fv3rfd1N2lTHeCvhROHBhuY.mp4`
  · 카메라 자동 명명: `IMG_*`, `DSC_*`, `Screenshot_*`, `Screen Shot *`
  · 미디어 확장자(.mp4 / .mov / .jpg / .jpeg / .png / .heic / .webp / .gif)
    이면서 파일명에 사업명·프로젝트명이 명시되지 않은 경우
  · 단순 영숫자 토큰만 있는 파일 (예) `IMG_8933.jpeg`, `1152.PDF`)
이 파일들은 본문/메타데이터 없이는 절대 어느 사업 폴더로도 보낼 수 없다.
"AI 연구 사업 때문에 발생한 거 같으니까" 같은 추정으로 사업 폴더에 넣지 마라.

**키워드 일치 조건 (assignments 의 hard rule):**
  파일명 토큰 중 *최소 한 개*가 그 카테고리의 name 또는 keywords 와 의미적으로
  일치해야만 assignments 에 넣는다. 일치하는 토큰이 없으면 deferred 로 보낸다.

원칙:
- 추상 라벨 금지 ("문서", "보고서", "프레젠테이션", "기타").
- 확신이 70% 이하면 deferred 로 보낸다 — 잘못된 분류보다 다음 단계로 넘기는 것이 낫다.
- 같은 사업이라도 산출물/구매·계약/회계/미디어는 별도 카테고리로 분리한다.
  특히 "NTS_eTaxInvoice", "세금계산서", "영수증", "구매 계약", "발주",
  "거래명세서" 가 들어간 파일은 사업 산출물 폴더에 절대 넣지 마라.
...
def build_stage_a(files: list[dict], *, reclassify_mode: bool = False) -> str:
    body = json.dumps(files, ensure_ascii=False, indent=2)
    return (
        f"{STAGE_A_SYSTEM}{_maybe_hint(reclassify_mode)}\n\n"
...
def build_compact_discover(files: list[dict], *, reclassify_mode: bool = False) -> str:
    body = json.dumps(files, ensure_ascii=False)
    return (
        f"{COMPACT_SYSTEM}{_maybe_hint(reclassify_mode)}\n\n"
...
def build_compact_merge(
    ...
    *,
    ...
    return f"{COMPACT_SYSTEM}{_maybe_hint(reclassify_mode)}\n\n{instr}\n\nbatches:{body}"
